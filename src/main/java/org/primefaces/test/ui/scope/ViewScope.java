package org.primefaces.test.ui.scope;
 ...
 *
 * All rights retained by the original owner
 */
public class ViewScope implements Scope, Serializable, HttpSessionBindingListener {
    ...
    public Object get(String name, ObjectFactory objectFactory) {
        ...
        LOGGER.trace("Requesting bean from ViewScope [{}]", name);

        Map<String, Object> viewMap = FacesContext.getCurrentInstance().getViewRoot().getViewMap();
        //noinspection SynchronizationOnLocalVariableOrMethodParameter
        synchronized (viewMap) {
            if (viewMap.containsKey(name)) {
                LOGGER.trace("Returning existing bean from ViewScope [{}]", name);
                return viewMap.get(name);
            } else {
                LOGGER.trace("Creating new bean from ViewScope [{}]", name);
    ...
    public void clearFromListener(ViewScopeViewMapListener listener) {
        LOGGER.trace("Removing listener from map");
